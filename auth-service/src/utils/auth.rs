...
pub const TOKEN_TTL_SECOND: i64 = 600;

fn generate_auth_token(email: &Email) -> Result<String, GenerateTokenError> {
    let delta = chrono::Duration::try_seconds(TOKEN_TTL_SECOND)
...
pub async fn validate_token(
    ...
    match banned_token_store.read().await.check_token(token).await {
        ...
        Err(TokenStoreError::TokenBanned) => Err(jsonwebtoken::errors::Error::from(
